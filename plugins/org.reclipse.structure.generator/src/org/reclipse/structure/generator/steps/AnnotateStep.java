...
public class AnnotateStep
{
	...
	public void generate(Activity activity, PSPatternSpecification pattern)
	{
		...
		if (hasAntecedentPatterns)
		{
			...
			StorydrivenUtil.addTransition(nodeCache.get(ADD_ANNOTATION), nodeCache.get(STORE_ANTECEDENTS), EdgeGuard.END);

			// transit back to core match
			StorydrivenUtil.addTransition(nodeCache.get(STORE_ANTECEDENTS), nodeCache.get(MATCH_CORE_NODES));
		}
		else
		{
			// transit back to core match
			StorydrivenUtil.addTransition(nodeCache.get(ADD_ANNOTATION), nodeCache.get(MATCH_CORE_NODES), EdgeGuard.END);
		}
		...
		if (hasAdditionalElements)
		{
			...
			{
				StorydrivenUtil.addTransition(nodeCache.get(ADD_ANNOTATION), nodeCache.get(FIND_ADDITIONAL_ELEMENTS), EdgeGuard.END);
			}
