...
public class TriggerManager
{
	...
	private static final Comparator<PSObject> ID_AND_INCOMMING_COMPARATOR = new Comparator<PSObject>()
	{
		@Override
		public int compare(PSObject one, PSObject two)
		{
			if (one.getIncoming().size() == two.getIncoming().size()) {
				return ID_COMPARATOR.compare(one, two);
			} else {
				return one.getIncoming().size() < two.getIncoming().size() ? -1 : 1;
			}
		}
	};
...
//		EClass triggerType = chooser.getTrigger(types);
		...
		for (PSObject object : SpecificationUtil.getObjects(pattern))
		{
			if (SpecificationUtil.isNormal(object) && types.contains(object.getInstanceOf()))
			{
		...
		Collections.sort(possibilities, ID_AND_INCOMMING_COMPARATOR);
